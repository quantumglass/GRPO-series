"""
简化版 DeepScaler + LoRA 训练（早期实验入口）。

推荐新用户使用 train_unified.py；本脚本保留用于复现早期实验。
用法: uv run python train_deepscaler_legacy.py --config configs/train_deepscaler_legacy.yaml
"""
