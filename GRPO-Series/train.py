"""
Countdown 任务训练入口（与上游 GRPO-Zero 兼容）。

用法:
    uv run python train.py --config configs/train_countdown.yaml
    uv run python train.py --config configs/train_countdown_24gb.yaml
"""
...
def main(config_path: str):
    ...
    kl_coef = config["training"].get("kl_coef", 0.0)
    if kl_coef > 0:
        print(f"KL penalty enabled with kl_coef={kl_coef}")
    ...
    for step, batch in enumerate(train_dataloader, start=1):
        ...
        loss = results["loss"]
        kl_loss = results.get("kl_loss")
        mean_response_len = np.mean(
            [len(episode.generated_token_ids) for episode in episodes]
        )
        kl_msg = f", kl_loss: {kl_loss:.4f}" if kl_loss is not None else ""
        print(
